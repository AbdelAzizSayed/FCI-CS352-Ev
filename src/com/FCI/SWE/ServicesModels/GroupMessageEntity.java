...
import com.FCI.SWE.Services.PostServices;
...
public class GroupMessageEntity {
	...
	public boolean sendGroupMessage(String sender) {
		...
		notifyGroupMessageMemmbers(Emails, groupMesgId ,sender);
		return true ;
	}

	public void notifyGroupMessageMemmbers(String memebrsEmails, String groupMesgId ,String sender)
	//we send sender to avoid sending to himself
	{
		DatastoreService datastore = DatastoreServiceFactory
				.getDatastoreService();
		Query gaeQuery = new Query("notifications");
		Scanner s = new Scanner(memebrsEmails);
		s.useDelimiter(",");		
		while (s.hasNext()) 
		{
			...
			if (RecEmail.equals(sender))
				continue;
			Entity notification = new Entity("notifications");//the id in db will be random number
			notification.setProperty("notiClass", "ReadGroupMessageCommand"); //name of class to handle reaction
			notification.setProperty("notifID", groupMesgId);//groupmessageID
	...
	public boolean readGroupMessage() {
		...
		for (Entity entity : pq.asIterable()) {
			...
			if (entity.getProperty("notifID").equals(groupMessageID)
					&& entity.getProperty("RecEmail").equals(recEmail)) {
